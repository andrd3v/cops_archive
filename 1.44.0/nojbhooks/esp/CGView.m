...
- (id)initWithFrame:(UIWindow *)drawWindow
{
    ...
    if (self) {
        ...
        waters = (unsigned int (*)[4])malloc(0);
        waterCount = 0;
    }
...
CGContextSetAlpha(context, 255);
    ...
    for (int i = 0; i < waterCount; i++) {
        ...
        CGRect rect = CGRectMake(waters[i][0], waters[i][1], waters[i][2], waters[i][3]);
        
        // Создайте UIBezierPath для прямоугольника
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:rect];
        
        // Установите цвет заливки с прозрачностью 40%
        UIColor *fillColor = [UIColor colorWithRed:1.0 green:0.65 blue:0.0 alpha:0.3];
        [fillColor setFill];
        
        // Заполните прямоугольник цветом
        [path fill];
        
        // Установите оранжевый цвет обводки
        [[UIColor orangeColor] setStroke];
        path.lineWidth = ([[UIScreen mainScreen] bounds].size.width * 1/600);
        
        // Отрисуйте обводку прямоугольника
        [path stroke];
        
        // Добавьте текст внутри прямоугольника
        NSString *text = @"andr hack";
        UIFont *font = [UIFont fontWithName:@"San Francisco" size:12]; // Замените "YourCustomFontName" на название вашего шрифта
        NSDictionary *attributes = @{NSFontAttributeName: font, NSForegroundColorAttributeName: [UIColor whiteColor]};
        CGSize textSize = [text sizeWithAttributes:attributes];
        CGPoint textOrigin = CGPointMake(CGRectGetMidX(rect) - textSize.width / 2, CGRectGetMidY(rect) - textSize.height / 2);
...
-(void) addwatermark:(float)x y:(float)y w:(float)w h:(float)h {
    waterCount++;
    waters = (unsigned int (*)[4])realloc(waters, waterCount*sizeof(unsigned int[4]));
    waters[waterCount-1][0] = x;
    waters[waterCount-1][1] = y;
    waters[waterCount-1][2] = w;
    waters[waterCount-1][3] = h;
}
