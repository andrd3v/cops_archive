...
template <uintptr_t... Idx> struct vxCplEncryptedString<vxCplIndexList<Idx...> > {
    ...
    constexpr ALWAYS_INLINE vxCplEncryptedString(const char* const Str) : Value{ vxCplEncryptChar(Str[Idx], Idx)... } {}
...
// Compile-time int string encryption macro, converts back to int
#define ENCRYPTINT(Str) atoi(ENCRYPT(Str))

// Compile-time float string encryption macro, converts back to float
#define ENCRYPTFLOAT(Str) strtof(Str, NULL)
